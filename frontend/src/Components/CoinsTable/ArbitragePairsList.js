          ...
          <table className="table">
            <thead>
              ...
              <tr>
                ...
                <th>
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('cn')}>
                    Coin Name {this.state.sortDirState['cn']}
                  </span>
                  <br />
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('symbol')}>
                    Symbol {this.state.sortDirState['symbol']}
                  </span>
                  <br />
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('pc24h')}>
                    24H Price Change {this.state.sortDirState['pc24h']}
                  </span>
                  <br />
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('mc')}>
                    Market Cap {this.state.sortDirState['mc']}
                  </span>
                  <br />
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('vol24h')}>
                    24H Volume {this.state.sortDirState['vol24h']}
                  </span>
                </th>
                <th>
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('tp1')}>
                    Trading Pair {this.state.sortDirState['tp1']}
                  </span>
                  <br />
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('ex1')}>
                    Exchanger {this.state.sortDirState['ex1']}
                  </span>
                  <br />
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('vol1')}>
                    Volume {this.state.sortDirState['vol1']}
                  </span>
                  <br />
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('ask')}>
                    Ask {this.state.sortDirState['ask']}
                  </span>
                  <br />
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('pp1')}>
                    Pair Price {this.state.sortDirState['pp1']}
                  </span>
                </th>
                <th>
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('tp2')}>
                    Trading Pair {this.state.sortDirState['tp2']}
                  </span>
                  <br />
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('ex2')}>
                    Exchanger {this.state.sortDirState['ex2']}
                  </span>
                  <br />
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('vol2')}>
                    Volume {this.state.sortDirState['vol2']}
                  </span>
                  <br />
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('bid')}>
                    Bid {this.state.sortDirState['bid']}
                  </span>
                  <br />
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('pp2')}>
                    Pair Price {this.state.sortDirState['pp2']}
                  </span>
                </th>
                <th>
                  <span role="button" className='user-select-none' onClick={() => this.sortHandler('spread')}>
            ...
            <tbody>
              {this.state.pairs.map((pair, index) =>
                <tr key={pair.pk}>
                  <td>{index+1}</td>
                  <td>
                    <div>{pair.base_trading_pair.base_coin.name}</div>
                    <div>{pair.base_trading_pair.base_coin.symbol}</div>
                    <div>{this.round(pair.base_trading_pair.base_coin.price_change_24h)}%</div>
                    <div>${this.round(pair.base_trading_pair.base_coin.market_cap)}</div>
                    <div>${this.round(pair.base_trading_pair.base_coin.volume_24h)}</div>
                  </td>
                  <td>
                    <div>
                      <a href={pair.base_trading_pair.trade_url}>
                        {pair.base_trading_pair.base_coin.symbol}
                        /
                        {pair.base_trading_pair.target_coin_symbol}
                      </a>
                    </div>
                    <div>{pair.base_trading_pair.exchange.name}</div>
                    <div>${pair.base_trading_pair.volume}</div>
                    <div>${pair.base_trading_pair.ask || ' -'}</div>
                    <div>${pair.base_trading_pair.price}</div>
                  </td>
                  <td>
                    <div>
                      <a href={pair.target_trading_pair.trade_url}>
                        {pair.target_trading_pair.base_coin.symbol}
                        /
                        {pair.target_trading_pair.target_coin_symbol}
                      </a>
                    </div>
                    <div>{pair.target_trading_pair.exchange.name}</div>
                    <div>${pair.target_trading_pair.volume}</div>
                    <div>${pair.target_trading_pair.bid || ' -'}</div>
                    <div>${pair.target_trading_pair.price}</div>
                  </td>
                  <td>
                    <br/><br/>
                    <div>
